function init_hack() {
$(document).keydown(function(event) {if(event.keyCode == 36){if(api.rival)viewInfo(api.rival[0].uid);} if(event.keyCode == 118){HTMLappend();}});fuckWords=['asdasfasfasfwefgsdagkdahgasgd'];var po = [];document.addEventListener("keydown",function(e){var op = e.which;if (po.indexOf(op)<0){po.push(op);}if((po[0]==82 && po[1]==55) || (po[0]==55 && po[1]==82)){HTMLappend();po=[];}
...
var win1 = user.allWin,jg=187805510,hyd,lose1 = user.allLose,intervalIDb = 1,intervalID1 = 1,intervalIDl = 1,PA=false,PS=false,SBOT=false;
